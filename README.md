...
Currently, LIBCx provides the following extensions:
 ...
 - Improved positional read/write operations provided by `pread()` and `pwrite()` APIs to guarantee atomic behavior. kLIBC emulates these functions using a pair of `lseek` and `read()/write()` calls in non-atomic manner which leads to data corruption when accessing the same file from multiple threads or processes.
